...
file_to_save3 = os.path.join("Resources", "zillow_data_neighborhood.json")
file_to_save4 = os.path.join("Resources", "zillow_coord_data.js")
...
# Build file to read neighborhoods
with open(file_to_load, "r") as f:
    reader = csv.reader(f)
    header = next(reader)
    list = []
...
with open(file_to_load, "r") as f:
    ...
    for row in reader:
        data.append(    
            ...
            "neighborhood":row[22],
            "website_url":row[17],
            "coordinates":[float(row[7]), float(row[9])]
            }
        )
